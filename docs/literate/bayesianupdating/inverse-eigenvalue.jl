...
A matrix of this form can represent different problems, like the stiffness matrix describing a tuned mass damper system. In this example we assume the fixed values $\theta_11$ = 0.5 and $\theta_2$ = 1.5 for the variables.
...
The a priori knowledge of $\theta_1$ and $\theta_2$ is that they take values between 0.01 and 4. The likelihood function used for this problem is a bivariate Gaussian function with the off-diagonal terms of the covariance matrix equal to 0 and the diagonal terms corresponding to the standard deviation of the respective noise terms.
...
#  A scatter plot of the resulting samples shows convergence to two distinct points. In difference to the transitional Markov Chain Monte Carlo algorithm, the standard Metropolis-Hastings algorithm would have only identified one of the two points.
